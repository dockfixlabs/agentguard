"""Benchmark: scan all frameworks with full pipeline (FP filter + classifier)."""
import sys, os, time
sys.path.insert(0, '.')
...
frameworks = [
    'camel', 'qwen-agent', 'langchain-audit', 'crewai-audit',
    'autogen-audit', 'llama-index-audit', 'dify-audit',
]

print(f"{'Framework':<22s} {'Files':>5s} {'Raw':>5s} {'Post-FP':>7s} {'C':>4s} {'H':>4s} {'M':>4s} {'FP-Rem':>6s} {'Conf':>5s} {'Invest':>6s}")
print("-" * 85)
